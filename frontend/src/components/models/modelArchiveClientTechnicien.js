...
const ModelArchiveClientTechnicien = ({technicienId}) => {
    ...
    const archiveList = async (event) => {
        ...
        try {
          const response = await fetch(
            `/api/Techniciens/archiveClients/${technicienId}`,
            {
              method: "PATCH",
          ...
          <Typography id="keep-mounted-modal-description" sx={{ mt: 2 }}>
            {error ? (
             <div className="error">{error}</div>
                  ...
                  <p>Etes-vous sur d'archiver la liste des coupures et retablissements de ce technicien</p>
            )}
          </Typography>
...
export default ModelArchiveClientTechnicien;
